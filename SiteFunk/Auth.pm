...
=cut
    ...
    if ( ! $query->cookie('CGISESSID') ) {
        ...
        # Not sure if I should redirect here. There's been no database update
        # but then again, I have modified the session.
...
sub old_reset_password {

    my $self = shift ;

#
# The only error that we will echo back to the user is when they haven't
# provided a userid. We want to be silent on the other possible errors so as not
# to give hackers a route to discover valid userids and/or emails.
#

    sub _old_reset_password {
   ...
   $self -> check_rm (
    ...
    my $input_hash = { user_userid => $query -> param ( 'user_userid' ) } ;
...
#
    ...
    } elsif ( grep /^user_exists$/ , $results -> invalid ( 'user_userid' ) ) {

#
# The userid entered does not exist, silently ignore the request
#

    } elsif ( grep /^user_confirmed$/ , $results -> invalid ( 'user_userid' ) ) {
